...
func TestStateString(t *testing.T) {
	...
	out := "2b 7e 15 16 28 ae d2 a6 ab f7 15 88 9 cf 4f 3c "
	if x := s.String(); out != x {
...
func TestStateXor(t *testing.T) {
	s := State([]byte{0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff})
	xor := State([]byte{0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f})
	out := State([]byte{0x00, 0x10, 0x20, 0x30, 0x40, 0x50, 0x60, 0x70, 0x80, 0x90, 0xa0, 0xb0, 0xc0, 0xd0, 0xe0, 0xf0})
	if s.Xor(xor); !bytes.Equal(s, out) {
		t.Errorf("Sate xor failed with %v", s)
	}
}
