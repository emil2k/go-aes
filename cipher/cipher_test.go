...
func TestAddRoundKey(t *testing.T) {
	...
	c.state = state.State([]byte{0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff})
...
func TestGetRoundKey(t *testing.T) {
	...
	c := Cipher{key: *key.NewKey(4, ck)}
	if out0 := c.GetRoundKey(); !bytes.Equal(out0, ck) {
		t.Errorf("Get round key for round 0 failed with %v", out0)
	}
	if out1 := c.GetRoundKey(); !bytes.Equal(out1, r1) {
...
func TestEncrypt(t *testing.T) {
	...
	if x := Encrypt(4, 10, in, ck); !bytes.Equal(x, out) {
