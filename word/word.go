...
// Word represents an array of bytes
type Word []byte
...
// Xor xors a input's bytes into the Word, if the input contains more bytes they are ignored
func (w Word) Xor(input []byte) {
	for i := range w {
		if i < len(input) {
			w[i] ^= input[i]
		} else {
...
// sub is an array that represents the forward s-box
var sbox = [256]byte{
...
// invSbox is an array that represents the inverse s-box
